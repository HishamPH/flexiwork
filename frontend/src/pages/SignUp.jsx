...
const SignUp = () => {
  ...
  const {
    ...
    onSubmit: async (values, action) => {
      ...
      try {
        const res = await axios.post("/api/user/signup", rest, {
          headers: {
      ...
      <div className="bg-white p-8 shadow-lg max-w-md w-full">
        ...
        <div className="mb-2">
          ...
          <Menu as="div" className="relative">
            <MenuButton className='flex items-center justify-between p-3 bg-blue-gray-600 rounded-lg cursor-pointer border border-gray-300 shadow-sm' type='button'>
                ...
                <MenuItem>
                  {({ focus }) => (
                    <button
                      ...
                      className={`block px-4 py-2 text-lg w-full text-left ${focus ? 'bg-gray-100 text-gray-900' : 'text-gray-700'}`}
                    >
                      candidate
                    </button>
                  )}
        ...
        {errors.terms && touched.terms ? (
          ...
          <button type="submit"
            className="w-full bg-blue-500 text-white py-3 shadow-md hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500" disabled={isSubmitting}>
              Sign Up
          </button>
        </form>
